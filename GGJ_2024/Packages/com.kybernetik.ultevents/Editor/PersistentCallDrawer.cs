...
namespace UltEvents.Editor
{
    ...
    internal sealed class PersistentCallDrawer : PropertyDrawer
    {
        ...
        private static List<Type> GetSuportedTypes()
        {
            ...
            var tasks = new Task<List<Type>>[assemblies.Length];
            for (int i = 0; i < assemblies.Length; i++)
            {
                var assembly = assemblies[i];
                var task = tasks[i] = new Task<List<Type>>(() => GetSupportedTypes(assembly));
                task.Start();
            }
            ...
            _SupportedTypes.Sort((a, b) => a.FullName.CompareTo(b.FullName));
        ...
        {
            var supported = new List<Type>(128);
